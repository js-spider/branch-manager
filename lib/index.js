...
unwrapExports(completionDetection_plugin);
completionDetection_plugin.completionDetectionPlugin;
...
src$3.default.formatters.L = (value) => String(utils.filterHasLength(value) ? value.length : '-');
src$3.default.formatters.B = (value) => {
...
function createLog() {
    return src$3.default('simple-git');
}
...
var clone$1 = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", { value: true });
...
unwrapExports(clone$1);
clone$1.cloneMirrorTask;
clone$1.cloneTask;
...
var move$2 = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", { value: true });
...
const {cloneTask, cloneMirrorTask} = clone$1;
...
var webpack = {
	name: "webpack",
	repo: "git@github.com:js-spider/webpack-test.git",
	expired: "1h",
	whiteList: [
		"master",
		"test",
...
var stash$1 = /*#__PURE__*/Object.freeze({
	__proto__: null,
	default: stash,
	webpack: webpack
});

var require$$0 = getCjsExportFromNamespace(stash$1);
...
function writeJsonSync(stash){
  lib.writeJsonSync(path.resolve(__dirname, './stash.json'), stash, {spaces: 2});
}
...
function searchSingle(remote, stash){
  ...
  if(current){
    ...
    return gitCtr.getBranchListWithDate(remote).then((res)=>{
      ...
      if(list.length){
        console.log(source.blue('待删除分支列表:'));
        list.forEach((item)=>{
          console.log(source.blue(item.date), '::::', source.blue(item.branch));
        });
        return list
      }else {
        console.log(source.bgGreenBright(`${remote}无过期分支`));
        return []
      }
    })
  }else {
      ...
      if(Array.isArray(list) && list.length){
        ...
        list.forEach((item)=>{
          ...
          promiseArr.push(gitCtr.deleteRemoteBranch(remote,branchName).then(res =>{
            console.log(source.green(`${remote}的远程分支${branchName}已删除`));
          }));
        });
...
function onList(){
  ...
  list.forEach(item =>{
    ...
    console.log(source.greenBright(formatName), source.green(' >>> ' + item.repo));
  });
}
...
function onAdd(name,repo,whiteList,expired){
  ...
  if(stash[name]){
    console.log(source.red(`仓库名称重复: 已存在仓库<${name}>:[${stash[name].repo}]`));
    return ;
  }
  const repoExist = list.find(item => item.repo === repo);
  if(repoExist){
    console.log(source.red(`仓库repo重复: 已存在仓库<${repoExist.name}>:[${repoExist}]`));
    return ;
  }
  ...
  gitCtr.addRemote(stash).then((added)=>{
    if(Array.isArray(added) && added.length){
      console.log(source.green('添加成功,查看列表如下:'));
      onList();
    }else {
      console.log(source.red('未添加任何remote'));
    }
  });
}
...
function onDelete(remote){
  ...
  if(remote){
    console.log(source.cyan(`正在检查${remote}的过期分支 ...`));
    return tools.deleteSingle(remote, stash)
  }else {
    console.log(source.cyan(`检查本地所有remotes...`));
    return gitCtr.getRemotes().then(res =>{
      if(Array.isArray(res)){
        let arr = [];
        res.forEach(item =>{
          console.log(source.green(item.name));
          arr.push(item.name);
        });
...
function onDeleteRemote(remote){
  ...
  gitCtr.deleteRemoteLocal(remote).then(res =>{
    ...
    delete stash[remote];
    tools.writeJsonSync(stash);
    console.log(source.red(`本地remote ${remote} 删除成功`));
  });
}
...
var name = "@js-spider/branch-manage";
var version = "1.0.5";
var description = "";
...
var author = "pangchao <pangchao@zetyun.com>";
var license = "MIT";
var dependencies = {
	chalk: "^4.1.2",
	commander: "11.1.0",
